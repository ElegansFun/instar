...
const cards = [
  card("hero", `<div class="split"><div>${head("A persistent world, settled on Solana", "A cage of flies,<br>each driven by a complete<br><i>wiring diagram.</i>")}<p class="lede">Every fly runs every neuron of one real fruit fly's nervous system. Nothing scripted. Every fly an NFT. Every epoch hashed to Solana.</p></div><div class="art">${plate()}</div></div>`),
  ...
  card("lifecycle", `${head("How it works", "A fly's life, and where the money goes.")}<div class="steps"><div><b>01 Born</b>a crossover of its parents' weights with a few mutations; minted and offered</div><div><b>02 Bought</b>60% vault · 15% metabolism · 15% pool · 10% to the parent</div><div><b>03 Lives</b>every eighth epoch a quarter of the pool is shared by points: survival, foraging, energy, age, maturity, births, generation</div><div><b>04 Dies</b>NFT burned; 40% to living heirs · 10% to the keeper · 35% metabolism · 15% pool</div></div>`),
  ...
  card("custody", `${head("Custody", "Two ways in. Both stated plainly.")}<div class="cols two"><div><h3>Instar account</h3><p>A Solana keypair the world process holds for you, encrypted under your PIN. Quick. A trust assumption: the operator holds the key. Withdraw everything to any address at any time.</p></div><div><h3>Your wallet</h3><p>Connect any Solana wallet. The page builds each transaction from the program's IDL; your wallet signs; the world never sees your key.</p></div></div>`),

  card("verify", `${head("Verification", "Don't take the operator's word.")}<div class="steps"><div><b>01</b>Every 2,400 ticks the engine hashes the whole world: every fly, every position, every neuron it touched (FNV-1a, 64 bit).</div><div><b>02</b>The operator posts that hash to the World account on Solana.</div><div><b>03</b>Anyone fetches the snapshot, replays the same ticks with the same engine, hashes, and compares.</div></div><div class="cmd">INSTAR_URL=https://instarcage.com npm run verify:epoch</div><p class="note">Same bytes, honest world. Different bytes, caught. Latest: ${v ? `${v.verdict} epoch ${v.epoch}, hash ${v.hash}` : "no result posted"}.</p>`),
  ...
  card("recovery", `${head("If the operator disappears", "Every lamport has a way out.")}<div class="timeline"><div><b>day 0</b>the operator goes quiet. Credits are already pull-only: nobody needs anyone.</div><div><b>day 90</b>anyone may begin wind-down. Keepers reclaim their flies' vaults as credit. Treasuries sweep to the recovery address fixed at creation.</div><div><b>day 180</b>whatever nobody came back for goes to recovery; every account closes for its rent.</div></div><p class="note">The operator can also wind down deliberately, any day. No lamport is stranded by design.</p>`),
