// Create the World account: init_world(recovery). Idempotent; prints the
// existing world if it is already there. Run from Windows:
...
if (await chain.worldExists()) {
  ...
  console.log(`world already exists: operator ${w.operator.toBase58()}, recovery ${w.recovery.toBase58()}, next_id ${w.nextId}`);
} else {
  ...
  const sig = await chain.initWorld(recovery);
